...
if __name__ == "__main__":
    ...
    log_path = f"log/train_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    ...
    if os.path.exists('policy_net.pth'):
        ...
        target_net.load_state_dict(policy_net.state_dict())
        log('Stage 1 skipped. Loaded policy_net.pth')
    else:
        log(f"{'='*60}")
        ...
        log('Stage 1 complete. Saved policy_net.pth')
    ...
    torch.save(policy_net.state_dict(), 'policy_net_ft.pth')
    log('Stage 2 complete. Saved policy_net_ft.pth')
